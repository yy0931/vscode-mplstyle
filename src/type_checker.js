...
/**
 * Returns [documentation, type checker]
 * @returns {[string, (value: string) => boolean]}
 */
    ...
    switch (signature.kind) {
        case "0 <= x < 1": {
            ...
            if (type.endsWith("_or_None")) {
                ...
                return [`"none" | ${child[0]}`, (x) => child[1](x) || x.toLowerCase() == "none"]
            }
            if (type.endsWith("list")) {
                // key: val1, val2
                const child = getTypeChecker({ kind: "validate_", type: type.slice(0, -"list".length) })
                return [`List[${child[0]}]`, (x) => x.split(",").map((v) => v.trim()).every((v) => child[1](v))]
            }
            ...
            switch (type) {
                case "dpi":
                    ...
                    return [`int | "figure"`, (x) => x === "figure" || typeof json5Parse(x) === "number"]
                case "fontsize": {
                    ...
                    return [`float | ${values.map((x) => JSON.stringify(x)).join(" | ")}`, (x) => values.includes(x) || typeof json5Parse(x) === "number"]
                } case "fontweight": {
                    ...
                    return [values.map((x) => JSON.stringify(x)).join(" | "), (x) => values.includes(x)]
                } case "float":
                ...
                case "int": {
                    ...
                    return ["bool", (x) => ["t", "y", "yes", "on", "true", "1", "f", "n", "no", "off", "false", "0"].includes(x.toLowerCase())]
                case "string":
                    return [`str`, (x) => true]
                case "any":
                    return [`any`, (x) => true]
